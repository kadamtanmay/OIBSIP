...
function convertTemperature(values) {
	switch (values.fromUnit) {
		case 'C':
			...
			if (values.toUnit === 'K') return +values.fromTemperature + 273.15;
		case 'F':
			if (values.toUnit === 'C') return (+values.fromTemperature - 32) / 1.8;
			if (values.toUnit === 'K') return (+values.fromTemperature - 32) / 1.8 + 273.15;
		case 'K':
			if (values.toUnit === 'C') return +values.fromTemperature - 273.15;
			if (values.toUnit === 'F') return (+values.fromTemperature - 273.15) / 1.8 + 32;
	}
}
